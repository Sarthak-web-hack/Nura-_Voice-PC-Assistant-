...
class Jarvis:
    ...
    def greet_user(self):
        ...
        if current_hour < 12:
            greeting = "Good morning sarthak"
        elif current_hour < 18:
            greeting = "Good afternoon sarthak"
        else:
            greeting = "Good evening sarthak"
    ...
    def handle_code_requests(self, user_input):
        ...
        if create_match:
           ...
           # filepath = os.path.join(os.getcwd(), filename)
            ...
            subprocess.Popen([r"C:\Users\91965\AppData\Local\Programs\Microsoft VS Code\Code.exe", filepath])
    ...
    def is_youtube_open(self, gw=None):
        try:
    ...
    def focus_youtube(self, gw=None):
        try:
            for window in gw.getAllWindows():
                if window.title and "youtube" in window.title.lower():
